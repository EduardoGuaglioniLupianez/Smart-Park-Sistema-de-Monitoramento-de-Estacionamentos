/*
    ...
    Boa noite, Vivian e Clara!
	Não temos certeza se esta é a última entrega oficial de Banco de Dados,
    se não for, estamos abertos a feedbacks e correções.
...
CREATE TABLE usuario(
	...
	nivel_acesso_usuario VARCHAR(20) DEFAULT('Usuário Comum'),
	CONSTRAINT Cnivel_acesso_usuario CHECK(nivel_acesso_usuario IN('Administrador','Cliente','Usuário Comum'))
);
...
CREATE TABLE vaga(
    ...
    setor_vaga CHAR(5), -- S1-02
    status_atual VARCHAR(15),
    CONSTRAINT cTipo CHECK(tipo_vaga IN('Comum', 'PCD', 'Idoso', 'Moto', 'VIP')),
    CONSTRAINT cStatus CHECK(status_atual IN('Livre', 'Ocupada'))
);

CREATE TABLE sensor(
	id_sensor INT PRIMARY KEY AUTO_INCREMENT,
    id_vaga INT UNIQUE NOT NULL,
    modelo_sensor CHAR(5) DEFAULT 'LM393',
